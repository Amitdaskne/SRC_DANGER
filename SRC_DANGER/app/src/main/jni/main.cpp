//shared by @DangerIndiaSrc , JOIN TG :- @DangerIndiaSrc & DANGER INDIA SRC 🛘
#include "DangerSDK/SDK.hpp"
using namespace SDK;
using namespace std;
...
bool IsInsideFOVs(int x, int y, int glWidth, int glHeight, float Fov) {
...
if (Canvas) {
    char Menunamee[256];
    sprintf(Menunamee, OBFUSCATE("AMIT HCK"));
    ...
    tslFontUI->LegacyFontSize = screenWidth * 0.017f; // Adjust multiplier as needed
    ...
    DrawOutlinedTextFPS(Canvas, Menunamee, {menunameePositionX, menunameePositionY}, COLOR_YELLOW, COLOR_IN, true);
...
if (g_LocalPlayer && g_PlayerController) {
        ...
        Config.PlayerESP.ItemEsp = false;
...
FixGameCrash();
   ...
   *(uintptr_t *)&MessageBoxExt = UE4 + 0x7FC531C;
...
__attribute__((constructor)) void _init() {
   ...
   // pthread_create(&Ptid, 0, Define_thread, 0);
...
Java_com_androidx_multidex_MultiDexApplication_api(JNIEnv *env, jclass) {
    return SIGNATUR ? env->NewStringUTF("ok") : (exit(1), (jstring)0);
}

JNIEXPORT void JNICALL
Java_com_androidx_multidex_MultiDexApplication_SIGNATUR(JNIEnv *, jobject, jint code, jboolean val) {
    if (code == 750099000) SIGNATUR = val;
    else exit(1);
}
